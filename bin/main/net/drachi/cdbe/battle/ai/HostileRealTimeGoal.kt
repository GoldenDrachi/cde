...
class HostileRealTimeGoal(private val pokemonEntity: PokemonEntity) : Goal() {
    ...
    override fun canUse(): Boolean {
        ...
        val runtimeState = net.drachi.cdbe.battle.utility.SpawnManager.getEntityHostility(pokemonEntity)
        val savedHostility = pokemonEntity.pokemon.persistentData.getString("cdbe_hostility")
        val isHostile = runtimeState == net.drachi.cdbe.battle.utility.HostilityState.HOSTILE || 
                        savedHostility == "hostile" || 
                        (runtimeState == null && savedHostility.isEmpty() && 
                         net.drachi.cdbe.battle.utility.SpawnManager.getDimensionHostility(pokemonEntity.level().dimension().location().toString()) == net.drachi.cdbe.battle.utility.HostilityState.HOSTILE)
    ...
    override fun tick() {
        ...
        cooldownTicks = (rtMove.cooldownTurns * net.drachi.cdbe.config.ConfigManager.config.turnToSecondsRatio * 20).toInt()
