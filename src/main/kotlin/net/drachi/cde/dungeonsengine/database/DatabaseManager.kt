package net.drachi.cde.dungeonsengine.database
...
import net.drachi.cde.dungeonsengine.data.ActiveDungeon
...
object DatabaseManager {
    ...
    fun initialize(config: net.drachi.cde.config.GlobalConfig) {
        val hikariConfig = HikariConfig()
        
        if (config.databaseType.equals("sqlite", ignoreCase = true)) {
            val dbFile = File(FabricLoader.getInstance().configDir.resolve("cde").toFile(), "dungeons.db")
        ...
        try {
            ...
            createTables()
            CDE.logger.info("Successfully connected to ${config.databaseType} database.")
        } catch (e: Exception) {
            CDE.logger.error("Failed to connect to database!", e)
        }
    }
        ...
        getConnection()?.use { conn ->
            conn.prepareStatement(sql).use { stmt ->
                ...
                while (rs.next()) {
                    ...
                    if (!net.drachi.cde.dungeonsengine.data.DungeonManager.configs.containsKey(configId)) {
            ...
            conn.prepareStatement(sql).use { stmt ->
                ...
                while (rs.next()) {
                    ...
                    if (config != null) {
                        ...
                        CDE.logger.warn("Skipped loading dungeon $instanceId because config '$configId' is missing.")
                    }
                }
            }
        }
