...
import net.drachi.cde.dungeonsengine.registry.ModItems
...
class BossDoorBlock(properties: Properties) : Block(properties) {
    ...
    override fun useItemOn(
        ...
        if (!level.isClientSide) {
            ...
            val dungeon = net.drachi.cde.dungeonsengine.data.DungeonManager.getActiveDungeon(player)
            if (dungeon != null) {
                // If this is the boss floor, check if enemies are alive
                if (dungeon.config.endFloorType == net.drachi.cde.dungeonsengine.data.EndFloorType.BOSS && dungeon.currentFloor == dungeon.config.amountOfFloors) {
                    val floorOriginZ = dungeon.originZ
                    val floorOriginX = dungeon.originX + ((dungeon.currentFloor - 1) * 1000)
                    
                    val dim = net.drachi.cde.dungeonsengine.generation.DungeonGrid.gridSizeForRooms(1) // Boss floor has 1 maxRooms
                    ...
                    val remainingEnemies = level.getEntitiesOfClass(net.minecraft.world.entity.Mob::class.java, bounds) {
                        it.tags.contains("cde_spawned") && it.isAlive
                    }
                    
                    if (remainingEnemies.isEmpty()) {
                        breakConnectedDoors(level, pos)
                        player.displayClientMessage(net.minecraft.network.chat.Component.translatable("message.cde.door_opens").withStyle(net.minecraft.ChatFormatting.GREEN), true)
                        return ItemInteractionResult.SUCCESS
                    } else {
                    ...
                    player.displayClientMessage(net.minecraft.network.chat.Component.translatable("message.cde.door_locked").withStyle(net.minecraft.ChatFormatting.RED), true)
                    return ItemInteractionResult.SUCCESS
                }
            } else if (stack.`is`(ModItems.TREASURE_KEY)) {
