...
class TemplateMapper(
    ...
    fun loadPools(hallwayIds: List<ResourceLocation>, endIds: List<ResourceLocation>) {
        ...
        for (id in hallwayIds) {
            ...
            val cat = when (effectiveJigsaws.size) {
                ...
                2 -> {
                    ...
                    if (dx == 0 && dz != 0) {
                        ...
                        CDE.logger.info("Auto-Corrected Z-Straight: $id. p1=$p1->${expectedP2Facing.opposite}, p2=$p2->$expectedP2Facing")
                        ...
                        CDE.logger.info("Auto-Corrected X-Straight: $id. p1=$p1->${expectedP2Facing.opposite}, p2=$p2->$expectedP2Facing")
                        ParsedTemplate.Category.STRAIGHT
                    } else if (dx == 0 && dz == 0) {
            ...
            when (cat) {
                ...
                else -> CDE.logger.warn("Skipped un-categorizable template: $id")
            }
        }
    ...
    fun mapNodes(
        ...
        floorConfig: net.drachi.cde.dungeonsengine.data.FloorConfig,
        ...
        for (x in 0 until grid.width) {
            for (z in 0 until grid.height) {
                ...
                if (pt == null) {
                    CDE.logger.warn("No junction template found to cover open directions $openDirs at ($x, $z)")
                    continue
                }
                ...
                if (intersectsBlockedCell(piece, grid) || intersectsRoomPhysically(piece, roomPieces)) {
                    CDE.logger.warn("Skipping junction at ($x, $z) because it intersects room")
                    continue
                }
                
                CDE.logger.info("Placed Node: template=${pt.template} rot=$rotation pos=$worldPos jigsaws=${piece.parsedJigsaws.map { "${it.pos.x},${it.pos.y},${it.pos.z}:${it.facing}" }}")
        ...
        CDE.logger.info("Node mapping complete: placed $placed nodes.")
        return nodeMap
    }
    ...
    fun mapWires(
        ...
        CDE.logger.info("Wire mapping complete: filled $wiresPlaced wires with $segmentsPlaced segments.")
    }
    ...
    private fun fillGapJigsawToJigsaw(
        ...
        while (iter < 200) {
            ...
            if (!placedPiece) {
                ...
                CDE.logger.warn("Gap fill failed! Start: ${start.pos}, Target: ${end.pos}, Remaining distSq: $distSq. Pool piece distances: $availableDistances")
                break
            }
        }
    ...
    private fun tryConnectToAdjacentPiece(
        ...
        for (candidate in pool.shuffled(kotlin.random.Random(random.nextLong()))) {
            ...
            if (rot != null) {
                ...
                CDE.logger.info("Upgraded adjacent piece at ($cx, $cz) to $category to connect with exit at ${jig.pos}")
                return true
            }
        }
                ...
                if (targetCategory != null) {
                    ...
                    for (candidate in pool.shuffled(kotlin.random.Random(random.nextLong()))) {
                        ...
                        if (rot != null) {
                            ...
                            CDE.logger.info("Optimized node at cell $key: swapped with ${candidate.template.toString()} (reduced exits to ${connectedDirs.size})")
                            break
                        }
                    }
                }
    ...
    companion object {
        ...
        fun getPalettes(template: StructureTemplate): List<StructureTemplate.Palette> {
            return try {
                ...
                CDE.logger.error("Failed to get palettes field via reflection", e)
                emptyList()
            }
        }
        ...
        fun detectHazard(template: StructureTemplate): Boolean {
            ...
            for (palette in palettes) {
                for (block in palette.blocks()) {
                    if (block.state.`is`(net.drachi.cde.dungeonsengine.registry.ModBlocks.HAZARD)) {
                        return true
                    }
                }
            }
