...
object DungeonLogger {
    ...
    fun exportMap(placedPieces: List<StructurePiece>, filename: String) {
        if (placedPieces.isEmpty()) {
            CDE.logger.warn("DungeonLogger: No pieces to export.")
            return
        }
        ...
        if (width <= 0 || height <= 0 || width > 2000 || height > 2000) {
            CDE.logger.error("DungeonLogger: Invalid or excessively large bounds ($width x $height).")
            return
        }
        ...
        try {
            ...
            CDE.logger.info("DungeonLogger: Exported physical map to ${file.absolutePath}")
        } catch (e: Exception) {
            CDE.logger.error("DungeonLogger: Failed to write log file: ${e.message}")
        }
    }
}
