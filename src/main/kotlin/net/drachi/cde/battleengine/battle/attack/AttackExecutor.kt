...
object AttackExecutor {
    ...
    fun handleCastRequest(player: net.minecraft.server.level.ServerPlayer, partySlotIndex: Int, moveSlotIndex: Int) {
        ...
        if (cobblemonMove != null) {
            val ppMult = net.drachi.cde.battleengine.config.BattleEngineConfigManager.config.ppMultiplier
            if (ppMult > 0) {
                var cost = 1.0 / ppMult
                while (cost >= 1.0) {
    ...
    fun executePhaseNow(
        ...
        val ctx = MoveContext(
            ...
            world = world,
            showLog = (caster as? net.minecraft.server.level.ServerPlayer)?.let { net.drachi.cde.battleengine.battle.attack.PlayerCombatManager.isCombatLogEnabled(it) } ?: false,
            showDetail = (caster as? net.minecraft.server.level.ServerPlayer)?.let { net.drachi.cde.battleengine.battle.attack.PlayerCombatManager.isDetailedDamageEnabled(it) } ?: false,
            heldItem = heldItem,
    ...
    private fun playMoveSounds(ctx: MoveContext) {
        ...
        // 1. Play Cry
        var playedCry = false
        val activeEntity = net.drachi.cde.battleengine.battle.utility.EntityUtil.findLivingEntityByUUID(world.server, ctx.pokemonStats.uuid)
    ...
    private fun filterAndLimitTargets(targets: Set<net.minecraft.world.entity.LivingEntity>, ctx: MoveContext): List<net.minecraft.world.entity.LivingEntity> {
        val filtered = targets.filter { entity ->
            ...
            ctx.phase.hitsFriendlies || !net.drachi.cde.battleengine.api.BattleEngineApi.isFriendly(ctx.caster, entity)
        }
    ...
    private fun applyDamageToPokemon(ctx: MoveContext, targetEntity: com.cobblemon.mod.common.entity.pokemon.PokemonEntity, targetCount: Int) {
        ...
        if (!net.drachi.cde.battleengine.config.BattleEngineConfigManager.config.isPvpEnabled && defenderStats.getOwnerUUID() != null) {
        ...
        if (result.breakdown.movePower > 0.0) {
            ...
            if (ownerUUID != null) {
                ...
                if (owner != null && net.drachi.cde.battleengine.battle.attack.PlayerCombatManager.isCombatLogEnabled(owner)) {
    ...
    private fun applyDamageToPlayer(ctx: MoveContext, targetEntity: ServerPlayer, targetCount: Int) {
        if (!net.drachi.cde.battleengine.config.BattleEngineConfigManager.config.isPvpEnabled) {
            val attackerOwner = (ctx.caster as? net.minecraft.world.entity.player.Player)?.uuid ?: (ctx.caster as? com.cobblemon.mod.common.entity.pokemon.PokemonEntity)?.pokemon?.getOwnerUUID()
            ...
            if (result.breakdown.movePower > 0.0) {
                ...
                if (CombatStateManager.hasVolatileStatus(targetEntity.uuid, "substitute") && ctx.phase.bypasses?.ignoresSubstitute != true) {
                    ...
                    if (targetEntity is net.minecraft.world.entity.Mob) {
                        val state = net.drachi.cde.battleengine.battle.utility.SpawnManager.getEntityHostility(targetEntity)
                        if (state != net.drachi.cde.battleengine.battle.utility.HostilityState.PEACEFUL) {
                ...
                if (net.drachi.cde.battleengine.battle.attack.PlayerCombatManager.isCombatLogEnabled(targetEntity)) {
    ...
    private fun processPostAttackItems(ctx: MoveContext) {
        ...
        val itemStrategy = net.drachi.cde.battleengine.battle.item.ItemRegistry.getItem(ctx.heldItem)
        ...
        if (ctx.blunderPolicyTriggered && ctx.heldItem != "cobblemon:blunder_policy") {
            net.drachi.cde.battleengine.battle.item.ItemRegistry.getItem("cobblemon:blunder_policy")?.onPostAttack(ctx)
        }
        if (ctx.throatSprayTriggered && ctx.heldItem != "cobblemon:throat_spray") {
            net.drachi.cde.battleengine.battle.item.ItemRegistry.getItem("cobblemon:throat_spray")?.onPostAttack(ctx)
        }
        if (ctx.lifeOrbTriggered && ctx.heldItem != "cobblemon:life_orb") {
             net.drachi.cde.battleengine.battle.item.ItemRegistry.getItem("cobblemon:life_orb")?.onPostAttack(ctx)
        }
    }
    ...
    private fun processRecharge(ctx: MoveContext) {
        ...
        if (ctx.phase.attackType == net.drachi.cde.battleengine.battle.attack.AttackTypeEnum.SELF && ctx.phase.attackDurationTurns > 0) { DelayedActionManager.applyRecharge(ctx.caster.uuid, ctx.phase.attackDurationTurns, ctx.caster, ctx.phase.attackMessage) }
    }
