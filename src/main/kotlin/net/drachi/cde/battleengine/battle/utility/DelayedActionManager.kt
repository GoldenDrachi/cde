...
object DelayedActionManager {
    ...
    fun queuePhase(executionId: UUID, casterEntity: net.minecraft.world.entity.LivingEntity, pokemonStats: Pokemon, move: RealTimeMove, moveTemplate: MoveTemplate, phase: MovePhase, delayTicks: Long) {
        ...
        phase.particles?.let { pData ->
            if (pData.type.startsWith("cobblemon:") && delayTicks > 0) {
                net.drachi.cde.battleengine.util.ParticleUtil.spawnEntityParticle(casterEntity.level() as net.minecraft.server.level.ServerLevel, pData.type, casterEntity)
            }
        }
    }
    ...
    fun applyRecharge(playerUuid: UUID, durationTurns: Float, casterEntity: net.minecraft.world.entity.LivingEntity, attackMessage: String?) {
        ...
        val durationMs = (durationTurns * net.drachi.cde.battleengine.config.BattleEngineConfigManager.config.turnToSecondsRatio * 1000).toLong()
    ...
    fun tick(server: MinecraftServer) {
        ...
        while (iterator.hasNext()) {
            ...
            val player = net.drachi.cde.battleengine.battle.utility.EntityUtil.findLivingEntityByUUID(server, queued.casterUuid)
                    ...
                    queued.phase.particles?.let { pData ->
                        if (!pData.type.startsWith("cobblemon:")) {
                            ...
                            for (i in 0 until pData.count) {
                                ...
                                net.drachi.cde.battleengine.util.ParticleUtil.spawnParticle(level, pData.type, rx, ry, rz, 1, 0.0, 0.0, 0.0, 0.0)
                            }
                        }
                    }
                }
        ...
        while (repeatingIterator.hasNext()) {
            ...
            if (currentTime >= rep.nextFireTimeMs) {
                val player = net.drachi.cde.battleengine.battle.utility.EntityUtil.findLivingEntityByUUID(server, rep.casterUuid)
                if (player != null && player.isAlive) {
                    // Fire next hit
