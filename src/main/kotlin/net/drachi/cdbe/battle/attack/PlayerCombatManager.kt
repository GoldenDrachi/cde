...
import net.drachi.cdbe.config.GameRulesManager
...
object PlayerCombatManager {
    ...
    fun canCast(player: ServerPlayer, requestedSlot: Int, moveId: String): Boolean {
        ...
        if (requestedSlot != -1 && state.activePartySlot != -1 && state.activePartySlot != requestedSlot) {
            ...
            if (ConfigManager.config.enablePlayerMorph) {
        ...
        if (targetMon != null) {
            ...
            if (heldItem == "cobblemon:assault_vest") {
                val rtMove = net.drachi.cdbe.battle.attack.MoveRegistry.getMove(moveId)
            ...
            if (isChoiceItem && moveId != "cobblemon:neutral_attack") {
                ...
                if (lockedMove != null && lockedMove != moveId) {
                    val moveName = net.drachi.cdbe.battle.attack.MoveRegistry.getMove(lockedMove)?.cobblemonMoveId ?: lockedMove
    ...
    fun markCast(player: ServerPlayer, slotIndex: Int, moveId: String, cooldownTurns: Float, gcdMs: Long) {
        ...
        val baseCooldownMs = (cooldownTurns * ConfigManager.config.turnToSecondsRatio * 1000).toLong()
        ...
        net.drachi.cdbe.network.NetworkHandler.sendCooldownSync(player, moveId, expirationMs)
    }
    ...
    fun toggleTargetLock(player: ServerPlayer) {
        ...
        if (state.lockedTargetId != -1) {
            ...
            net.drachi.cdbe.network.NetworkHandler.sendSyncLockedTarget(player, -1)
            return
        }
        ...
        if (closestTarget != null) {
            ...
            net.drachi.cdbe.network.NetworkHandler.sendSyncLockedTarget(player, closestTarget.id)
        } else {
    ...
    fun handleAutoSwitch(player: ServerPlayer): Boolean {
        ...
        for (i in 0 until party.size()) {
            ...
            if (p != null && p.currentHealth > 0) {
                ...
                net.drachi.cdbe.network.NetworkHandler.sendForceSelectedSlot(player, i)
    ...
    fun syncPlayerState(player: ServerPlayer) {
        ...
        net.drachi.cdbe.network.NetworkHandler.sendPlayerStateSync(player, activeItem, lockedMoveId)
    }
